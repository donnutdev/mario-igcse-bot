	...
	async execute(
		...
		switch (interaction.options.getSubcommand()) {
			case "create": {
				...
				if (lockTime === now) {
					const activeLockdown = existingLockdowns.find(
						(lockdown) => lockdown.locked,
					);
					if (activeLockdown) {
						await interaction.editReply({
							content:
								"This channel is already locked. Please run `/lockdown remove` to remove the active lockdown first.",
						});
						return;
					}
				}
				...
				if (hasOverlap) {
					await interaction.editReply({
						content:
							"The specified time period overlaps with an existing scheduled lockdown. Please choose a different time period.",
					});
					return;
				}
			...
			case "bulk": {
				...
				for (const entry of data) {
					...
					if (activeLockdown && lockTime === now) {
						results.push(
							`<#${channelId}> - Channel is already locked`,
						);
						continue;
					}
					...
					const hasOverlap = existingLockdowns.some((existing) => {
						...
						return (
							(lockTime >= existingStart &&
								lockTime < existingEnd) ||
							(unlockTime > existingStart &&
								unlockTime <= existingEnd) ||
							(lockTime <= existingStart &&
								unlockTime >= existingEnd)
					...
					if (hasOverlap) {
						results.push(
							`<#${channelId}> - Time period overlaps with existing lockdown`,
						);
						continue;
					}
