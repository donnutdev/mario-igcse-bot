...
import type { ChemInfo, SynonymInfo } from "@/utils/apis/cheminfo";
import {
	determineBonding,
	fetchChemicalData,
...
export default class ChemInfoCommand extends BaseCommand {
	...
	async execute(
		...
		try {
			const res = await fetchChemicalData(
				name || formula,
				formula ? "fastformula" : "name",
			);

			if ("Fault" in res) {
				await interaction.editReply(
					`An error occurred: ${res.Fault.Message}`,
				);
				return;
			}
			...
			const synonymList =
				filteredSynonyms.length > 0
					? filteredSynonyms.slice(0, 5).join(", ")
					: "";
			...
			const iupacName =
				compound.props.find((p) => p.urn.label === "IUPAC Name")?.value
					.sval || "";

			const embed = new EmbedBuilder()
				.setTitle(
					`${filteredSynonyms[0] || iupacName} (${formulaLabel})`,
				)
				.setDescription(
					`For more information, [click here](https://pubchem.ncbi.nlm.nih.gov/compound/${cid})`,
				)
				.addFields(
					{
					...
					{
						name: "Charge",
						value: `${compound.charge}`,
						inline: true,
					},
					{
						name: "Synonyms",
						value: `${synonymList || "N/A"}`,
						inline: true,
					},
					{
						name: "Color",
						value: `${experimentalProperties.color || "N/A"}`,
						inline: true,
					},
					{
						name: "Physical Description",
						value: `${experimentalProperties.description || "N/A"}`,
						inline: true,
					},
				)
				.setColor("#FF0000");
			...
			await interaction.editReply(
				"Invalid formula or name (an error occurred)",
			);
		}
	}
}
